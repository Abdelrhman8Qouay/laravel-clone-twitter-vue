<template>
                        ...
                        <template #more>
                            <div class="flex items-center">
                                <Link title="Back"
                                    ...
                                    :href="route('tweets.index')" preserve-state>
                    ...
                    <div class="w-full flex overflow-hidden border-b border-b-slate-500 p-2">
                        ...
                        <div class="w-[calc(100%-55px)] flex-1 h-full mx-1">
                            ...
                            <div v-else-if="tweet.is_video === 'y'"
                                class="relative flex justify-center items-center w-full h-[650px] rounded-2xl mt-2 cursor-pointer"
                                id="videoContainer">
                                <video @mouseover="hoverVideo = true" @mouseleave="hoverVideo = false"
                                    class="max-w-full max-h-full rounded-2xl border border-gray-400" :src="tweet.file"
                                    autoplay :controls="hoverVideo" :muted="!mutedPrevent" loop @canplay="updatePaused"
                                    @playing="updatePaused" @pause="updatePaused">
                                    <p>Sorry, there's a problem playing this video. Please try using a different browser.
                                    </p>
                                </video>
                                <button
                            ...
                            <div class="text-white font-light text-sm mt-4 max-[320px]:text-xs">
                                <div class="break-words text-gray-500 m-1 p-0">{{ new
                                    Date(tweet.created_at).toLocaleString() }} · <span class="text-white font-semibold">{{
        formatNumber(tweet.viewed_count) }}</span> Views</div>
                            ...
                            <div v-if="user_auth" class="mt-2">
                                <NewReply :tweet="dataNewReply" :is_overlay="false" />
                            </div>
                        </div>
                    </div>
                    <!-- New Post Tweet -->
                    <ReplyTweet v-for="(reply, i) in repliesTweetPosted" :key="i" :tweet="reply"
                        ...
                        @getreply="(data) => { dataNewReply = data; newReplyShow = true }" />
    ...
    <!-- Replay Overlay -->
    <NewReply :tweet="dataNewReply" :is_overlay="true" @clickself="(bol) => newReplyShow = bol" v-if="newReplyShow" />
