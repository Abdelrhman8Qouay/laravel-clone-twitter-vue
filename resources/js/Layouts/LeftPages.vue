<template>
  <div
    class="flex-shrink min-w-max xl:flex-shrink-0 xl:basis-1/3 select-none border-r border-r-slate-400 h-screen lg:p-4 md:p-3 sm:p-2 max-sm:p-2 max-[512px]:h-max max-[512px]:w-screen max-[512px]:absolute max-[512px]:left-0 max-[512px]:bottom-0 max-[512px]:border-l-0 max-[512px]:border-t max-[512px]:border-t-slate-400 max-[512px]:p-2 max-[512px]:z-[5000]"
  >
    <div
    ...
    >
      <div
        class="w-full max-[512px]:flex max-[512px]:flex-row max-[512px]:items-center max-[512px]:justify-between"
      >
        <!-- twitter logo home -->
        <a
          href="/"
      ...
      >
        <img
          src="https://pbs.twimg.com/profile_images/1625184499582615563/XdG9pB_s_400x400.png"
          class="rounded-full inline-block w-[45px] h-[45px]"
          alt="user image"
        />
        <div
          class="text-white mx-2 xl:inline-block max-xl:hidden font-serif font-semibold"
        >
          <p class="font-bold">Qouaynt Que</p>
          <span class="text-slate-500 font-light">@AbdelrhmanQouay</span>
        </div>
        <DotsHorizontal
          class="xl:inline-block max-xl:hidden"
          fillColor="#fff"
          :size="25"
        />
      </a>
  ...
  >
    ...
    >
      <!-- Close Buttons Div -->
      <div
        class="flex flex-row items-center justify-between w-full text-start mb-4"
      >
        <Close
          @click="openTweet = false"
          class="block max-md:hidden cursor-pointer"
          fillColor="#fff"
          :size="25"
        />
        <ArrowLeft
      ...
      <div class="flex flex-row gap-3 w-full">
        ...
        <div class="w-full">
          ...
          >
            <span class="flex justify-center items-center w-full text-white"
              >Everyone <ChevronDown fillColor="#2563eb" :size="22"
            /></span>
            <ul
            ...
            >
              <span class="font-bold text-lg p-2 text-white"
                >Choose audience</span
              >
              <li class="py-2 px-2 hover:bg-[#d6d9db1f]">
                <div
                  class="flex flex-row justify-between items-center text-white font-bold text-lg"
                >
          ...
          <div class="w-full mb-2">
            ...
            <!-- images & videos -->
            <div
              v-if="typeFile.type === 'image' && showUploaded"
              class="relative min-w-full max-w-full rounded-2xl"
            >
              <button
                @click="closeImage"
                class="rounded-full cursor-pointer p-2 bg-[#0f1419bf] backdrop-blur-sm hover:bg-[#0f141973] absolute left-2 top-2 flex justify-center items-center transition duration-500"
              >
                <Close fillColor="#fff" :size="20" />
              </button>
              <img
              ...
              <span
                @click="closeImage"
                class="rounded-lg w-max p-1 bg-[#0f1419bf] backdrop-blur-sm absolute left-2 bottom-2 text-white uppercase text-xs"
              >
                {{ typeFile.ext }}
              </span>
              <div class="flex flex-row items-center gap-3 mt-1">
                <span
                  class="text-xs text-gray-500 font-extralight no-underline hover:underline"
                  ><AccountTag fillColor="#6b7280" :size="20" /> Tag
                  People</span
                >
                <span
                  class="text-xs text-gray-500 font-extralight no-underline hover:underline"
                  ><ListBoxOutline fillColor="#6b7280" :size="20" /> Tag
                  People</span
                >
              </div>
            </div>
            <div
              v-if="typeFile.type === 'video' && showUploaded"
              class="relative min-w-full max-w-full rounded-2xl"
            >
              <button
                @click="closeImage"
          ...
          >
            <span class="flex justify-center items-center w-full text-[#1d9bf0]"
              ><Earth class="mx-1" fillColor="#1d9bf0" :size="18" /> Everyone
              can replay</span
            >
            <ul
              class="hidden group-active:flex flex-col absolute left-0 top-[50px] bg-black shadow-sm shadow-white rounded-lg transition duration-200"
          ...
          <div class="flex flex-row justify-between items-center my-2">
            <div
              class="flex flex-row justify-between items-center gap-2 text-[#1d9bf0]"
            >
              <label
                for="imageTweet"
              ...
              >
                <input
                  ...
                  hidden
                />
                <ImageOutline fillColor="#1d9bf0" :size="18" />
              </label>
              <div
                class="hover:bg-[#0199ff1f] rounded-full p-2 flex justify-between items-center"
              >
  ...
  <div
    @click.self="openDiscardTweet = false"
    v-if="openDiscardTweet"
